...
$(ESP):
	...
	if [ -f "المقلع/شاهين.نواة" ]; then \
		mcopy -i $(ESP) \
			المقلع/شاهين.نواة \
			::المقلع/شاهين.نواة; \
	fi

	# نسخ أرشيف النظام (المقلع/النظام.tar)
	if [ -f "$(SYSARCHIVE)" ]; then \
		echo "نسخ النظام.tar"; \
		mcopy -i $(ESP) \
			$(SYSARCHIVE) \
			::المقلع/النظام.tar; \
...
$(ISO): $(ESP)
	...
	# نسخ صورة القرص (esp.img) فقط كـ efiboot.img
	# كل ملفات ومجلدات النظام (المقلع، البوتلودر...) موجودة بالفعل
	# داخل هذه الصورة، ولا داعي لتكرارها في جذر الآيزو
...
run: $(ESP) $(OVMF_VARS)
	qemu-system-x86_64 \
		...
		-drive if=pflash,format=raw,file=$(OVMF_VARS) \
		-device ahci,id=ahci \
		-drive id=disk,file=$(ESP),format=raw,if=none \
